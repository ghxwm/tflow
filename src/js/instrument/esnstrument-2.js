...
//    }
    ...
    function appendControlEndCall(node){
    		node = funCond(node);
				var cid = getCondIid();
				var code = "{"+logControlBeginFunName + "("+RP+"1);" + logControlEndFunName+"("+RP+"2);}";
				var blockStmt = replaceInStatement(code,cid,cid)[0];
				var stmtArr = blockStmt.body;
				blockStmt.body=[stmtArr[0],node,stmtArr[1]];
				transferLoc(blockStmt, node);
				//console.log('control:'+JSON.stringify(blockStmt));
				return blockStmt;
    }
    ...
    var visitorOps = {
        ...
        "ConditionalExpression": wrapConditionExpression,
        "IfStatement": appendControlEndCall,
        "WhileStatement": appendControlEndCall,
        "DoWhileStatement": appendControlEndCall,
