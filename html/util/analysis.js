...
//-------------------------------- Constants ---------------------------------
        ...
        function create_fun(f,unpackBase) {
        						//处理native方法的wrapper函数参数.
        				function wrapFunForNativeResult(f){
        								return		function(){
        													var ret = f.apply(getConcrete(this),arguments);
        													return getConcrete(ret);
        								}
        				}
            ...
            return function() {
                ...
                for (var i = 0; i<len; i++) {
                    a=arguments[i] = getConcrete(arguments[i]);
                    if(typeof a === 'function'){
                    				arguments[i]=wrapFunForNativeResult(a);
                    							}else if(isArr(a)){ //handle array argument
		...
		try{
										var base_c = getConcrete(this);
										if(isArr && unpackBase){
											for(var j = 0 ; j < base_c.length; j++){
												base_c[j] = getConcrete(base_c[j]);
												}
										}
                	var ret = f.apply(base_c,arguments);
		}catch(e){
			console.log('call uninstrument function error:'+f.name + "("+JSON.stringify(arguments)+")");
		}
		finally{return ret;}
            }
        }
        ...
        function getSymbolicFunctionToInvokeAndLog (f, isConstructor) {
						if(!f){
			console.log('call undefined function...');	return ;	
	}
        					if(isNative(f) || !HOP(f,SPECIAL_PROP2)){ //本地方法或者没有被inst的方法
        						return [isConstructor?f:create_fun(f,isUnpackBase(f)),true];
        								}
        					return [null,true];
        						function isUnpackBase(f){
        										if( f === Array.prototype.join || 0){
        											return true;
        										}
        											return false;
        						}
        ...
        function callAsConstructor(Constructor, args) {
            if (isNative(Constructor)) {//native构造器
                ...
                ret = getConcrete(Constructor.apply(inst, args));//调用父类构造函数,注意构造函数可以有返回值.即可以拦截new 表达式的返回值
        ...
        function invokeFun(iid, base, f, args, isConstructor) {
			...
			try{
				...
				console.log('invoke function error:'+g.toString().substring(0,10));
				throw e;			
			}						}
                }  else {
                    val = undefined;
        ...
        function F(iid, f, isConstructor) {
            return function() {
                var base = this,name=f.name?f.name:f.toString().substring(0,30);
		//try{console.log('call function:'+name+" ");}catch(e){console.log('call function '+f+"error");};
        ...
        function M(iid, base, offset, isConstructor) {
            return function() {
                var f = G(iid, base, offset),fc=getConcrete(f);
		if(fc == Function.prototype.apply || fc == Function.prototype.call){
			var arg0=arguments[0];
			//var join = Array.prototype.join;
			//console.log('call special function '+ f.name + " with arguments "+ join.call(arguments,','));
			if(fc == Function.prototype.apply)arguments=getConcrete(arguments[1]);
			else arguments = Array.prototype.slice.call(arguments,1);
			//console.log('call special function "'+ f.name + '" with arguments '+JSON.stringify(arguments));
			f = base;
			base = arg0;
		}else if(Element.prototype.setAttribute === fc){ //set dom attribute
			P(iid,base,arguments[0],arguments[1]);
		}else if(Element.prototype.getAttribute == fc){
			G(iid,base,arguments[0]);
		}
		try{
			var name=fc.name?fc.name:fc.toString().substring(0,30);
		}catch(e){
			console.log('get function name error');
		}
		//try{console.log('call method:'+name+"");}catch(e){console.log('call method '+f+"error");};
                return invokeFun(iid, base, f, arguments, isConstructor);
            };
        }
   ...
   function R(iid, name, val, isGlobal) {
	...
	function Cb(iid){sandbox.analysis.controlBegin(iid);}

	function Ce(iid){sandbox.analysis.controlEnd(iid);}
        			//
