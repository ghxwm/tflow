...
//        N_LOG_FUNCTION_ENTER_NORMAL = 18,
    ...
    var global_names = {document:true,window:true,navigator:true,screen:true,location:true,history:true,Object:true,Array:true,Number:true,String:true};
...
//    }
    ...
    function wrapReadWithUndefinedCheck(node, name) {
    	if(global_names[name]){return wrapRead(node,createLiteralAst(name),createIdentifierAst(name), false, true);}
    		var ret = replaceInExpr(
        ...
        function handleFun(node) {
            ...
            if (node.type === 'FunctionDeclaration') {
                oldScope.addVar(node.id.name, "defun", node.loc);
                						currentScope.addVar('arguments','arg');
