...
if (typeof Allmonitor !== "object ") {
    Allmonitor = (function() {
        ...
        D = screen,
        A = window,
        e = A.performance || A.mozPerformance || A.msPerformance || A.webkitPerformance,
        n = false,
        y = [],
        ...
        function m() {
            ...
            if (q.addEventListener) {
                N(q, "DOMContentLoaded ",
                function i() {
                    q.removeEventListener("DOMContentLoaded ", i, false);
                    J()
                })
            } else {
                if (q.attachEvent) {
                    q.attachEvent("onreadystatechange ",
                    function i() {
                        if (q.readyState === "complete ") {
                            q.detachEvent("onreadystatechange ", i);
                            J()
                        }
                    });
                    if (q.documentElement.doScroll && A === A.top) { (function i() {
                            if (!n) {
                                try {
            ...
            if ((new RegExp("WebKit ")).test(d.userAgent)) {
                P = setInterval(function() {
                    if (n || /loaded|complete/.test(q.readyState)) {
                        clearInterval(P);
                        J()
                    }
                },
        ...
        function g(Q, P) {
            ...
            q.getElementsByTagName("head ")[0].appendChild(i)
        }
        function t() {
            var i = "";
            try {
                i = A.top.document.referrer
            } catch(Q) {
                if (A.parent) {
                    try {
                        i = A.parent.document.referrer
                    } catch(P) {
                        i = ""
                    }
                }
...
/*(?:[^@]+@)?([^:/#]+)"),
        ...
        function O(P) {
            P = P && P.text ? P.text: P;
            if (!l(P)) {
                var i = q.getElementsByTagName("title");
                if (i && s(i[0])) {
                    P = i[0].text
                }
            }
        ...
        function x(ak, aL) {
            var T = G(q.domain, A.location.href, t()),
            a5 = u(T[0]),
            bj = T[1],
            aS = T[2],
            aQ = "GET",
            ...
            function a0(bs, bp, bo, br, bn, bq) {
                ...
                q.cookie = bs + "=" + k(bp) + (bo ? ";expires=" + bm.toGMTString() : "") + ";path=" + (br || "/") + (bn ? ";domain=" + bn: "") + (bq ? ";secure": "")
            }
            function af(bo) {
                if (aa) {
                    return 0
            ...
            function a1(bm) {
                try {
                    var bo = A.XMLHttpRequest ? new A.XMLHttpRequest() : A.ActiveXObject ? new ActiveXObject("Microsoft.XMLHTTP") : null;
                    bo.open("POST", S, true);
            ...
            function ax(bo, bn) {
                if (A.AMPlatform) {
                    bo = A.AMPlatform.util.base64Encode(bo)
                }
                var bm = new Date();
                if (!bc) {
                    if (aQ === "POST") {
            ...
            function ab(bq, bn, bm, bp, bo, br) {
                a0(aZ("ysp_id"), bq + "." + bn + "." + bm + "." + bp + "." + bo + "." + br, ag, bi, ac)
            }
            function Q() {
                var bn = new Date(),
                ...
                if (bp) {
                    ...
                    if (!aA) {
                        aA = av((d.userAgent || "") + (d.platform || "") + JSON.stringify(ba) + bm).slice(0, 16)
                    }
                    bo = ["1", aA, bm, 0, bm, "", ""]
                }
                return bo
            ...
            function P() {
                var bm = aa;
                aa = false;
                a0(aZ("ysp_id"), "", -86400, bi, ac);
                a0(aZ("ysp_ses"), "", -86400, bi, ac);
                a0(aZ("ysp_cvar"), "", -86400, bi, ac);
                a0(aZ("ysp_ref"), "", -86400, bi, ac);
                aa = bm
            }
            function at(bo, bM, bN, bq) {
                var bK, bn = new Date(),
                bw = Math.round(bn.getTime() / 1000),
                bP,
                bL,
                bs,
                bD,
                ...
                var bA = q.characterSet || q.charset;
                if (!bA || bA.toLowerCase() === "utf-8") {
                    bA = null
                }
                bu = bI[0];
                bm = bI[1];
                ...
                if (!bB) {
                    ...
                    if (!aB || !bu.length) {
                        for (bK in aw) {
                            if (Object.prototype.hasOwnProperty.call(aw, bK)) {
                                bu = B(bO, aw[bK]);
                                if (bu.length) {
                                    break
                                }
                        ...
                        for (bK in aq) {
                            if (Object.prototype.hasOwnProperty.call(aq, bK)) {
                                bm = B(bO, aq[bK]);
                                if (bm.length) {
                                    break
                                }
                    ...
                    bQ = b(aS);
                    bz = bJ.length ? b(bJ) : "";
                    if (bQ.length && !aP(bQ) && (!aB || !bz.length || aP(bz))) {
                        bJ = aS
                    }
                    if (bJ.length || bu.length) {
                        bt = bw;
                        bI = [bu, bm, bt, be(bJ.slice(0, br))];
                        a0(by, JSON.stringify(bI), aD, bi, ac)
                    }
                }
                bo += "&idsite=" + a9 + "&rec=1&r=" + String(Math.random()).slice(2, 8) + "&h=" + bn.getHours() + "&m=" + bn.getMinutes() + "&s=" + bn.getSeconds() + "&url=" + k(be(bO)) + (aS.length ? "&urlref=" + k(be(aS)) : "") + "&_id=" + bL + "&_idts=" + bD + "&_idvc=" + bs + "&_idn=" + bP + (bu.length ? "&_rcn=" + k(bu) : "") + (bm.length ? "&_rck=" + k(bm) : "") + "&_refts=" + bt + "&_viewts=" + bv + (String(bF).length ? "&_ects=" + bF: "") + (String(bJ).length ? "&_ref=" + k(be(bJ.slice(0, br))) : "") + (bA ? "&cs=" + k(bA) : "");
                var bp = JSON.stringify(aH);
                if (bp.length > 2) {
                    bo += "&cvar=" + k(bp)
                }
                for (bK in ba) {
                    if (Object.prototype.hasOwnProperty.call(ba, bK)) {
                        bo += "&" + bK + "=" + ba[bK]
                    }
                }
                if (bM) {
                    bo += "&data=" + k(JSON.stringify(bM))
                } else {
                    if (Z) {
                        bo += "&data=" + k(JSON.stringify(Z))
                    }
                }
                if (V) {
                    var bC = JSON.stringify(V);
                    if (bC.length > 2) {
                        bo += "&_cvar=" + k(bC)
                    }
                    for (bK in bG) {
                        if (Object.prototype.hasOwnProperty.call(bG, bK)) {
                            if (V[bK][0] === "" || V[bK][1] === "") {
                                delete V[bK]
                            }
                        }
                    }
                    a0(bR, JSON.stringify(V), ai, bi, ac)
                }
                if (az && an) {
                    bo += "&gt_ms=" + an
                } else {
                    if (az && e && e.timing && e.timing.requestStart && e.timing.responseEnd) {
                        bo += "&gt_ms=" + (e.timing.responseEnd - e.timing.requestStart)
                    }
                }
                ab(bL, bD, bs, bw, bv, s(bq) && String(bq).length ? bq: bF);
                a0(bx, "*", ai, bi, ac);
                bo += I(bN);
                if (aN.length) {
                    bo += "&" + aN
                ...
                if (aY) {
                    for (bv in aY) {
                        if (Object.prototype.hasOwnProperty.call(aY, bv)) {
                            ...
                            if (!s(aY[bv][3]) || String(aY[bv][3]).length === 0) {
                                aY[bv][3] = 0
                            }
                            if (!s(aY[bv][4]) || String(aY[bv][4]).length === 0) {
                                aY[bv][4] = 1
                            }
                            bu.push(aY[bv])
                        }
                    }
                    br += "&ec_items=" + k(JSON.stringify(bu))
                }
                br = at(br, Z, "ecommerce", bs);
                ax(br, aM)
            }
            function a2(bm, bq, bp, bo, bn, br) {
                if (String(bm).length && s(bq)) {
            ...
            function aF(bp, bq) {
                var bm = new Date(),
                bo = at("action_name=" + k(O(bp || ar)), bq, "log");
                ax(bo, aM);
                if (X && al && !aT) {
                    aT = true;
                    N(q, "click", a8);
            ...
            function ap(bm, bp, bn, bq) {
                var bo = at("search=" + k(bm) + (bp ? "&search_cat=" + k(bp) : "") + (s(bn) ? "&search_count=" + bn: ""), bq, "sitesearch");
                ax(bo, aM)
            }
            ...
            function ao(br) {
                ...
                if (!ay) {
                    for (bm = 0; bm < bp.length; bm++) {
                        bo = bp[bm];
                        if (Object.prototype.hasOwnProperty.call(q, bb(bo, "hidden"))) {
                            if (q[bb(bo, "visibilityState")] === "prerender") {
                                bq = true
                            }
                            break
                        }
                ...
                if (bq) {
                    N(q, bo + "visibilitychange",
                    function bn() {
                        q.removeEventListener(bo + "visibilitychange", bn, false);
                        br()
                    });
                    return
                }
            ...
            function aX(br) {
                var bp, bn, bm;
                while ((bp = br.parentNode) !== null && s(bp) && ((bn = br.tagName.toUpperCase()) !== "A" && bn !== "AREA")) {
                    br = bp
                }
                if (s(br.href)) {
            ...
            return {
                ...
                setCustomVariable: function(bn, bm, bq, bo) {
                    ...
                    if (bn > 0) {
                        bm = s(bm) && !l(bm) ? String(bm) : bm;
                        bq = s(bq) && !l(bq) ? String(bq) : bq;
                ...
                setDoNotTrack: function(bn) {
                    var bm = d.doNotTrack || d.msDoNotTrack;
                    bc = bn && (bm === "yes" || bm === "1");
                    if (bc) {
                        this.disableCookies()
                    }
                ...
                setEcommerceView: function(bp, bm, bo, bn) {
                    ...
                    aH[5] = ["_pkc", bo];
                    if (s(bn) && String(bn).length) {
                        aH[2] = ["_pkp", bn]
                    }
                    if ((!s(bp) || !bp.length) && (!s(bm) || !bm.length)) {
                        return
                    }
                    if (s(bp) && bp.length) {
                        aH[3] = ["_pks", bp]
        ...
        N(A, "beforeunload", L, false);
        m();
        Date.prototype.getTimeAlias = Date.prototype.getTime;
        E = new x();
        for (H = 0; H < _paq.length; H++) {
            if (_paq[H][0] === "setTrackerUrl" || _paq[H][0] === "setSiteId") {
                K(_paq[H]);
                delete _paq[H]
            }
        }
    ...
    var aa = win.document.getElementById('_allmobilizeAA');
    if (aa) return aa.parentNode.removeChild(aa)
})(window);
"*/
