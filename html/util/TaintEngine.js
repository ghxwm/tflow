...
(function(sandbox){
		...
		TLabel.prototype.isIntegrity = function(){
			return HOP(UMARK_VALUES,this.label);
		}
		...
		var nopLog = function empty(){};
		var consoleLog = console.log.bind(console);
		var htmlLog = function(){
			var count = 0;
			var d = window.top.document,w=window.top;
			var div = d.createElement("div");
			div.innerHTML="========information flow======";
			div.tflow = true;
		...
		},UMARK_VALUES=(function(){
			var map = {};
			for(var k in SLABEL.UMARK){
				if(HOP(SLABEL.UMARK,k)){
					map[SLABEL.UMARK[k]]=true;
				}
			}
			return map;
	})();
   ...
   var SPECIAL_PROP_OBJ = "*T$*";   //对象字面量标记
        ...
        //当前的间接流污点setter方法
        		...
        		this.backupTaintSet = function(iid){
        							this['bkp'+iid] = this.currentTaintSet();
       ...
       /*this.hasLocation = function(objName,propName){
        ...
        this.injectTaint = function(obj,prop,val){
        				...
        				if( policy.hasLocation(obj_name,prop_name) && curl != null){
        					...
        					this.checkTaintRead(val,obj_name,prop_name);
        					if(val instanceof ConcolicValue && !HOP(val,'dict')){val['dict']=[obj_name,prop_name];}
        							//check read
        				}
        }
        ...
        function getMetaInfo(val){if(val instanceof ConcolicValue)return val['dict'];}
        ...
        this.updateVariableTaintSetForCurrent = function(val) { // x = I + x.taints
        					...
        					return new ConcolicValue(getConcrete(val),ns,getMetaInfo(val));
        }
        
        this.wrapVariableTaintSet = function(val,ts){
        					var val_c = getConcrete(val),val_s = getSymbolic(val);
        					if(ts === val_s && ts)return val;
        					else if(!ts)ts = this.currentTaintSet();
        					return new ConcolicValue(val_c,ts,getMetaInfo(val));
        }
        var getVariableTaintSet = this.getVariableTaintSet = function(val){
        	...
        	this.addVariableTaint = function(v,t){
        					...
        					var nts ;
        					if(v instanceof ConcolicValue && !ts.has(t)){
        									nts = ts.copy();
        									nts.add(t);
        									v.setSymbolic(nts);
        							}
        ...
        this.annotateFunction = function (fn,tinfo){
        					...
        					fn[SPECIAL_PROP_URL]=this[SPECIAL_PROP_TAINT_ROOT]['url'];//函数定义时的url.
        ...
        this.checkWrite = function(val,base,offset){//base可以为None
        	if(HOP(val,SPECIAL_PROP_WITH)){
						...
						if(base_c !== undefined && base_c !== null){
							alias = base_c[SPECIAL_PROP_OBJ_ALIAS];
							if(alias == null){
								if(isDom(base_c))alias = base_c.nodeName;
								else return;
							}
						}
						if(/^window/.test(alias)){
							//debugger;
						}
       ...
       this.checkRead = function(base,offset,val){//检查是否可以将val赋值给base,offset.
       		...
       		this.checkTaintRead = function(val,objName,propName){
       						...
       						for(i in taints){
       									t = taints[i];
       									if(t.isConfidentiality()){ //scret taint
       										incCounter(TRC,path);
       ...
       this.extendTaintValue=function(val,TF,ref){
        ...
        this.literal = function(iid, val) {
        		if(val instanceof ConcolicValue)return val;
       ...
       function isDom(val){
        ...
        this.invokeFun = function(iid, f, base, args, val, isConstructor) {
           var f_c = getConcrete(f),val_c = getConcrete(val);
           
           if(!HOP(f_c,SPECIAL_PROP_FUN)){//未instrument的方法
        	   					if(val_c && isDom(val_c)){
        	   							val_c[SPECIAL_PROP_OBJ_ALIAS]=val_c.nodeName;
        	   							}
        ...
        this.getField = function(iid, base, offset, val) {
        			...
        			if(offset_c === 'caller' && typeof base_c === 'function'){
        								return this.updateVariableTaintSetForCurrent(base_c.caller.caller.caller);
        						}
        			...
        			if(offset_c == 'document'){
        				1;
        			}
        			var base_alias = base_c[SPECIAL_PROP_OBJ_ALIAS];
        			log('read object prop {0}.{1}  with taint set{2}'.format(base_alias,offset_c,getVariableTaintSet(ret)));
        			return ret;
        }
        ...
        this.readPre = function(iid, name, val, isGlobal) {  //read check
        					...
        					if(typeof val_c === 'object' && val_c != null && !val_c[SPECIAL_PROP_OBJ_ALIAS]) {
        												if(name == 'document'){
        																//	1;
        												}
        									if(name!='this')val_c[SPECIAL_PROP_OBJ_ALIAS]=name;	
        								}
        					if(val instanceof ConcolicValue && HOP(val,'dict')){
        										this.checkTaintRead(val,val['dict'][0],val['dict'][1]);
        								}
        ...
        this.writePre = function(iid, name, val,isLocal) {
        	var base = null,val_c = getConcrete(val);
        	if(HOP(val,SPECIAL_PROP_WITH)){
        					base=val[SPECIAL_PROP_WITH];
        				}
        	if(typeof val_c === 'object' && val_c != null && !val_c[SPECIAL_PROP_OBJ_ALIAS]) {
        ...
        this.write = function(iid, name, val) {
        	var val_c = getConcrete(val),tc=getVariableTaintSet(val);
						if(typeof val_c === 'function'){
								val_c = 'function literal';
						}
        	log('write variable {0} with value {1} and taint set{2}'.format(name,val_c,getVariableTaintSet(val)));
        	if(!tc)val=this.updateVariableTaintSetForCurrent(val);
        	return val;
        }
        ...
        this.binary = function (iid, op, left, right, result_c) {
            ...
            return this.wrapVariableTaintSet(result_c, taints);
        }
        ...
        this.unary = function (iid, op, left, result_c) {
            ...
            return this.wrapVariableTaintSet(result_c, ts);
        }
        ...
        this.conditional = function (iid, left, result_c) {
            return left;
        		...
        		this.controlEnd = function(iid){
        								if(iid)this.restoreTaintSet(iid);
        		}
        		...
        		this.forIn = function(iid,obj){
        						var obj_c = getConcrete(obj),ret = {},tk;
        									for(var k in obj_c){
        													if(HOP(obj_c,k) && !/^taint_/.test(k)){ //复制对象,不包括以taint_开头的属性
        																		tk = 'taint_' + k;
        																		if(HOP(obj_c,'taint_'+k)){ret[k]=obj_c[tk];}
        																		else{ret[k] = obj_c[k];}
        ...
        this.endExecution = function() {
        					...
        					for(objName in TRC){
        							it_obj = TRC[objName];
        							for(propName in it_obj){
        										it_prop = it_obj[propName];
        										for(url in it_prop){
        														cnt = it_prop[url];
