...
Douban.init_interest_form = function (b) {
    ...
    var c = function () {
        var n = $.trim(b.tags.value.toLowerCase()).split(" "),
            o = {};
        $.each(n, function (q, p) {
            if (p != "") {
                j(p);
    ...
    $(b).submit(function () {
        ...
        remote_submit_json(this, function (s) {
            var w = $("#dialog .shuo :input[type=checkbox]"),
                o = $("div#dialog form.movie-sns"),
                q = $("div#dialog form.book-sns");
            if (s.r != 0) {
                $("#saving").remove();
                $("#submits").show();
            ...
            if (w.length && w[0].checked) {
                ...
                if (o.length && s.cid) {
                    ...
                    var u = dui.Dialog({
                        title: "已保存为想看",
                        url: "/j/coupon_info?coupon_id=" + v,
                        autoupdate: true,
                        callback: function (x, y) {
                            $("a#btn-giveup, a#btn-close", y.node).bind("click", function () {
                                $(y.node).remove();
                                self.location.replace(self.location.href);
                                return false
                            });
                            $("a#btn-accept", y.node).bind("click", function (z) {
                                z.preventDefault();
                                $.post_withck("/ticket/coupon/get/" + p + "/accept?cid=" + v, {}, function () {
                                    var A = $(y.node);
                                    A.find("h3").html("你领取了这张优惠券");
                                    A.find(".coupon_tit").html("领取成功，可以到可用的优惠券 ( <a href='http://movie.douban.com/ticket/coupon/' target='_blank'>http://movie.douban.com/ticket/coupon/</a> ) 查看");
                                    A.find(".coupon").css("visibility", "hidden");
                                    A.find("#btns a").hide();
                                    A.find("#btns #btn-close").show()
                                });
                                return false
                            })
                        }
                    ...
                    if (q.length && s.book_pop_sync) {
                        ...
                        var r = dui.Dialog({
                            ...
                            autoupdate: true,
                            callback: function (x, y) {
                                $("a#btn-later", y.node).bind("click", function () {
                                    $("div.dui-dialog").remove();
                                    self.location.replace(self.location.href);
                                    return false
                                });
                                $("a#btn-never", y.node).bind("click", function (z) {
                                    z.preventDefault();
                                    $.post_withck("/settings/never_pop_sync_settings", {}, function () {
                                        $("div.dui-dialog").remove();
                                        self.location.replace(self.location.href)
                                ...
                                $("a.dui-dialog-close", y.node).bind("click", function () {
                                    self.location.replace(self.location.href);
                                    return false
                                })
                            }
...
this.set_overlay = function () {
    var c = $("#dialog"),
        d = c[0].offsetHeight + ($.browser.msie ? -2 : 16),
...
$(window).one("load", function () {
    ...
    var c = function (d) {
        ...
        $.get(typeof _MOREURL_REQ == "undefined" ? "/stat.html?" + d : _MOREURL_REQ + "?" + d, function () {
            if (b.length) {
                c(b.shift());
                localStorage.setItem("report", b.join("_moreurl_separator_"));
                return
            }
...
function _moreurl(b, e) {
    ...
    localStorage.setItem("report", (localStorage.getItem("report") || "") + "_moreurl_separator_" + d.join("&"))
}
...
Douban.init_post_link = function (b) {
    $(b).click(function (j) {
        ...
        if (h) {
            ...
            $('<form action="' + n + '" method="POST" target="' + (k || "_self") + '" sytle="display:none">' + r.join("") + "</form>").appendTo("body").submit()
        } else {
